...
```go
// 允许提升普通订单为VIP
func (om *OrderManager) UpgradeToVIP(orderID string) error {
    // 1. 从队列移除
    // 2. 修改类型和优先级
    // 3. 重新入队
}
...
```go
// 一次创建多个订单
func (om *OrderManager) CreateBatchOrders(count int, orderType OrderType) []*Order {
    orders := make([]*Order, count)
    for i := 0; i < count; i++ {
        orders[i] = om.CreateOrder(orderType)
    }
    return orders
}
```
...
```go
// PENDING超过5分钟自动取消
func (om *OrderManager) startTimeoutChecker() {
...
func (om *OrderManager) checkTimeouts() {
    // 检查pending队列
    // 超时订单自动取消
}
```
...
// 每个餐厅独立管理
func (rm *RestaurantManager) GetRestaurant(id string) *Restaurant
```
...
**Q1: 为什么不用数据库？**
A: 需求明确说明"No data persistence is needed"，使用内存存储即可。但可以提到扩展时可以加入Redis/PostgreSQL。
...
**Q3: WebSocket vs 轮询？**
A:
- WebSocket: 实时性好，服务器主动推送，连接开销小
- 轮询: 实现简单，但延迟高，服务器压力大
- 选择WebSocket更适合实时订单系统
...
func TestConcurrentOrderCreation(t *testing.T) {
    ...
    for i := 0; i < 100; i++ {
        ...
        go func() {
            defer wg.Done()
            om.CreateOrder(OrderTypeNormal)
        }()
    }
